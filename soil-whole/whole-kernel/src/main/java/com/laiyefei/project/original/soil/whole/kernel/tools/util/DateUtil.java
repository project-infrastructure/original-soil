 ...
 */
public abstract class DateUtil extends DateUtils implements IUtil {
    ...
    public static Date fuzzyConvert(String dateString) {
        if (Validator.isEmpty(dateString)) {
            return null;
        }
        // 清洗
        ...
        parts[0] = StringUtil.join(ymd, "-");
        if (parts.length == 1) {
            return DateUtil.convert2FormatDate(parts[0], DateUtil.FORMAT_DATE_Y4MD);
        }
        // 18:20:30:103
        ...
        parts[1] = StringUtil.join(hmsArray, ":");
        return DateUtil.convert2FormatDate(StringUtil.join(parts, " "), DateUtil.FORMAT_DATETIME_Y4MDHMS);
    }
