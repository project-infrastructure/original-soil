...
public abstract class StringUtil extends StringUtils {
    ...
    public static String cut(String input) {
        return cut(input, SystemConfig.getCutLength());
    }
    ...
    public static String toSnakeCase(String camelCaseStr) {
        ...
        for (char c : chars) {
            if (Character.isUpperCase(c)) {
                if (sb.length() > 0) {
                    sb.append(Constant.SEPARATOR_UNDERSCORE);
                }
            }
    ...
    public static String toLowerCaseCamel(String snakeCaseStr) {
        ...
        // 不包含_
        if (!snakeCaseStr.contains(Constant.SEPARATOR_UNDERSCORE)) {
            // 全大写直接return小写
        ...
        String[] words = snakeCaseStr.split(Constant.SEPARATOR_UNDERSCORE);
    ...
    public static boolean toBoolean(String strValue, boolean defaultBoolean) {
        if (Validator.notEmpty(strValue)) {
            return Validator.isTrue(strValue);
        }
