...
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding.parser.BaseConditionManager;
 ...
 */
@Slf4j
public class JoinConditionManager extends BaseConditionManager implements IAid {
    ...
    private static void parseJoinOn(AnnoJoiner joiner, List<Expression> expressionList) {
        ...
        joiner.setOnSegment(StringUtil.join(segments, " AND "));
        if (Validator.notEmpty(middleTableOnSegments)) {
            joiner.setMiddleTableOnSegment(StringUtil.join(middleTableOnSegments, " AND "));
        }
    }
    ...
    private static String formatColumn(Expression expression, AnnoJoiner joiner) {
        String annoColumn = StringUtil.toSnakeCase(expression.toString());
        if (expression instanceof Column) {
            // 其他表列
            if (annoColumn.contains(".")) {
                String tableName = StringUtil.substringBefore(annoColumn, ".");
                // 当前表替换别名
                if (tableName.equals("this")) {
                    annoColumn = "self." + StringUtil.substringAfter(annoColumn, "this.");
                } else if (tableName.equals("self")) {
                } else if (tableName.equals(joiner.getMiddleTable())) {
                    annoColumn = joiner.getMiddleTableAlias() + "." + StringUtil.substringAfter(annoColumn, ".");
                } else {
