 ...
 * the License.
 */
package com.laiyefei.project.infrastructure.original.soil.whole.kernel.controller;
...
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.pojo.dto.JsonResult;
 ...
 * @Desc : CRUD增删改查通用RestController-父类
 * @Version : v1.0.0.20200829
 * @Blog : http://laiyefei.com
 * @Github : http://github.com/laiyefei
 */
public class BaseCrudRestController<E extends BasePo> extends BaseController implements IController {
    ...
    protected <VO> JsonResult getViewObject(Serializable id, Class<VO> voClass) throws Exception {
        // 检查String类型id
        if (id instanceof String && !StringUtil.isNumeric((String) id)) {
    ...
    protected JsonResult updateEntity(Serializable id, E entity) throws Exception {
        ...
        if (entity.getId() == null) {
            ...
            if (Constant.FieldName.id.name().equals(pk)) {
                ...
                entity.setId(longId);
            } else if (BeanUtil.getProperty(entity, pk) == null) {
                BeanUtil.setProperty(entity, pk, id);
            }
        }
        // 执行更新资源前的操作
    ...
    protected JsonResult batchDeleteEntities(Collection<? extends Serializable> ids) throws Exception {
        ...
        if (success) {
            ...
            log.info("删除操作成功，{}:{}", getEntityClass().getSimpleName(), StringUtil.join(ids));
            return new JsonResult();
        } else {
            log.warn("删除操作未成功，{}:{}", getEntityClass().getSimpleName(), StringUtil.join(ids));
            return new JsonResult(Status.FAIL_OPERATION);
        }
    }
    ...
    private Map<String, Object> buildPKDataMap(E entity) {
        ...
        String pk = ContextHelper.getPrimaryKey(getEntityClass());
        Object pkValue = (Constant.FieldName.id.name().equals(pk)) ? entity.getId() : BeanUtil.getProperty(entity, pk);
        data.put(pk, pkValue);
        return data;
    }
    ...
    protected Class<E> getEntityClass() {
        if (this.entityClass == null) {
            this.entityClass = BeanUtil.getGenericityClass(this, 0);
            if (this.entityClass == null) {
