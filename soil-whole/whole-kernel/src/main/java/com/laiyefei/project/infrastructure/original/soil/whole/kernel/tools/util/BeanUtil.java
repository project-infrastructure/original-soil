...
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding.AcceptAnnoCopier;
...
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.pojo.po.BasePo;
 ...
 */
public abstract class BeanUtil implements IUtil {
    ...
    private static final Set<String> IGNORE_FIELDS = new HashSet<String>() {{
        add(Constant.FieldName.createTime.name());
    }};
    ...
    public static <T> List<T> convertList(List sourceList, Class<T> clazz) {
        if (Validator.isEmpty(sourceList)) {
    ...
    public static Object convertValueToFieldType(Object value, Field field) {
        ...
        if (Integer.class.getName().equals(type)) {
            ...
            return DateUtil.fuzzyConvert(StringUtil.valueOf(value));
        }
        return value;
    }
        ...
        try {
            for (T model : allLists) {
                ...
                if (Validator.isEmpty(fields)) {
                    //未指定字段，以id为key
                    key = getStringProperty(model, Constant.FieldName.id.name());
                }
    ...
    public static <T> List<T> buildTree(List<T> allNodes, Object rootNodeId) {
        if (Validator.isEmpty(allNodes)) {
            return null;
        }
        // 提取所有的top level对象
        List<T> topLevelModels = new ArrayList();
        for (T node : allNodes) {
            Object parentId = getProperty(node, Constant.FieldName.parentId.name());
            if (parentId == null || Validator.fuzzyEqual(parentId, rootNodeId)) {
                topLevelModels.add(node);
            }
            Object nodeId = getProperty(node, Constant.FieldName.id.name());
            if (Validator.equals(nodeId, parentId)) {
                throw new BusinessException(Status.WARN_PERFORMANCE_ISSUE, "parentId关联自身，请检查！" + node.getClass().getSimpleName() + ":" + nodeId);
            }
        }
        if (Validator.isEmpty(topLevelModels)) {
        ...
        for (T node : allNodes) {
            ...
            setProperty(node, Constant.FieldName.children.name(), children);
        }
        return topLevelModels;
    }
    ...
    private static <T> List<T> buildTreeChildren(Object parentId, List<T> nodeList) {
        ...
        for (T node : nodeList) {
            Object nodeParentId = getProperty(node, Constant.FieldName.parentId.name());
            if (nodeParentId != null && Validator.equals(nodeParentId, parentId)) {
                if (children == null) {
        ...
        if (children != null) {
            for (T child : children) {
                Object nodeId = getProperty(child, Constant.FieldName.id.name());
                List<T> childNodeChildren = buildTreeChildren(nodeId, nodeList);
                if (childNodeChildren == null) {
                    childNodeChildren = new ArrayList<>();
                }
                setProperty(child, Constant.FieldName.children.name(), childNodeChildren);
            }
        }
        return children;
    }
     ...
     * @return
     */
    public static String extractDiff(BasePo oldModel, BasePo newModel) {
     ...
     * @return
     */
    public static String extractDiff(BasePo oldModel, BasePo newModel, Set<String> fields) {
        ...
        for (Map.Entry<String, Object> entry : oldMap.entrySet()) {
            ...
            if (checkThisField && !oldValue.equals(newValue)) {
                result.put(entry.getKey(), StringUtil.join(oldValue, CHANGE_FLAG, newValue));
            }
            // 从新的map中移除该key
            if (newValueObj != null) {
        ...
        if (!newMap.isEmpty()) {
            for (Map.Entry<String, Object> entry : newMap.entrySet()) {
                ...
                if (fields == null || fields.contains(entry.getKey())) {
                    result.put(entry.getKey(), StringUtil.join("", CHANGE_FLAG, newValue));
                }
            }
        }
        oldMap = null;
    ...
    public static <E> List collectIdToList(List<E> objectList) {
        ...
        return collectToList(objectList, Constant.FieldName.id.name());
    }
    ...
    public static <E> void bindPropValueOfList(String setterFieldName, List<E> fromList, String getterFieldName, Map valueMatchMap) {
        if (Validator.isEmpty(fromList) || Validator.isEmpty(valueMatchMap)) {
            return;
        }
        try {
    ...
    public static List<Field> extractAllFields(Class clazz) {
        ...
        while (clazz != null) {
            ...
            if (Validator.notEmpty(fields)) { //被重写属性，以子类override的为准
    ...
    public static Class getGenericityClass(Object instance, int index) {
        ...
        if (Validator.isEmpty(types) || index >= types.length) {
            types = resolvableType.getSuperType().getGenerics();
        }
        if (Validator.notEmpty(types) && types.length > index) {
    ...
    public static Map<String, Object> convertKeyValueList2Map(List<KeyValue> keyValueList) {
        if (Validator.notEmpty(keyValueList)) {
