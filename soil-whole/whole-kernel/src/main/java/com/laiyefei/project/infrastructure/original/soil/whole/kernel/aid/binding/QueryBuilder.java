 ...
 * the License.
 */
package com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding;
...
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding.data.DataAccessAnnoCache;
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding.data.DataAccessCheckInterface;
...
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding.query.dynamic.DynamicJoinQueryWrapper;
import com.laiyefei.project.infrastructure.original.soil.whole.kernel.aid.binding.query.dynamic.ExtQueryWrapper;
 ...
 * @Desc : QueryWrapper构建器
 * @Version : v2.0.0.20200829
 * @Blog : http://laiyefei.com
 * @Github : http://github.com/laiyefei
 */
public class QueryBuilder implements IAid {
    ...
    private static <DTO> QueryWrapper<DTO> dtoToWrapper(DTO dto, Collection<String> fields) {
        ...
        if (Validator.isEmpty(fieldValuesMap)) {
        ...
        for (Map.Entry<String, Object> entry : fieldValuesMap.entrySet()) {
            Field field = BeanUtil.extractField(dto.getClass(), entry.getKey());
            ...
            if (query != null) {
                ...
                if (annoJoiner != null && Validator.notEmpty(annoJoiner.getJoin())) {
                    // 获取注解Table
    ...
    public static String getColumnName(Field field) {
        ...
        if (field.isAnnotationPresent(BindQuery.class)) {
            ...
            if (Validator.notEmpty(columnName)) {
                columnName = StringUtil.toSnakeCase(columnName);
            }
        } else if (field.isAnnotationPresent(TableField.class)) {
            columnName = field.getAnnotation(TableField.class).value();
        }
        return Validator.notEmpty(columnName) ? columnName : StringUtil.toSnakeCase(field.getName());
    }
    ...
    private static <DTO> LinkedHashMap<String, Object> extractNotNullValues(DTO dto, Collection<String> fields) {
        ...
        // 转换
        List<Field> declaredFields = BeanUtil.extractAllFields(dtoClass);
        for (Field field : declaredFields) {
            // 非指定属性，非逻辑删除字段，跳过；
            if (fields != null && !fields.contains(field.getName())) {
                //Date 属性放过
                if (!Validator.equals(field.getType().getName(), "java.util.Date")) {
                    continue;
                }
            }
            ...
            try {
                ...
                if (Validator.isEmpty(value)) {
                    String prefix = Validator.equals("boolean", field.getType().getName()) ? "is" : "get";
                    Method method = dtoClass.getMethod(prefix + StringUtil.capFirst(field.getName()));
            ...
            // 忽略逻辑删除字段
            if (Constant.FieldName.deleted.name().equals(field.getName())
