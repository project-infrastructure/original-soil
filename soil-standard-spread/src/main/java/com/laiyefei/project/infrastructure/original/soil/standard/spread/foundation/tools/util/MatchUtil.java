 ...
 */
public abstract class MatchUtil implements IUtil {
    private static final String MOBILE_PHONE_NUMBER_PATTERN = "^0{0,1}(13[0-9]|15[0-9]|14[0-9]|18[0-9])[0-9]{8}$";
    ...
    public static final boolean IsEmail(final String input) {
        if (JudgeUtil.IsNull(input)) {
            return false;
        }
        return input.matches(EMAIL_PATTERN);
    }

    public static final boolean IsPhoneNumber(final String input) {
        if (JudgeUtil.IsNull(input)) {
            return false;
        }
        return input.matches(MOBILE_PHONE_NUMBER_PATTERN);
    }
}
